 ...
 *
 * VERSION 3 — après deux revues techniques.
 *
 * Corrections apportées depuis la v2 :
 *   1. validatedByActor (format admin:<uuid>) SUPPRIMÉ : les comptes plateforme
 *      utilisent un integer generatedAlwaysAsIdentity, pas un UUID. Remplacé par
 *      validatedByAccountId avec clé étrangère réelle.
 *   2. Clé étrangère slug → novaluthProfilesTable.slug ajoutée : la colonne
 *      cible est une clé primaire, la relation peut donc être posée maintenant.
 *   3. Contraintes de dates étendues aux états post-proposition et post-envoi.
 *   4. Origine de production de l'accroche séparée de sa validation humaine :
 *      une accroche peut rester d'origine « ai » tout en étant validée.
 *   5. PROSPECTION_CRITICAL_CONSTRAINTS exporte des objets complets, compatibles
 *      avec expectedNonSn13Constraints de schema-check.ts.
 *   6. Chronologie complétée : repliedAt et closedAt comparés à sentAt.
 *   7. Longueurs bornées sur tous les champs libres, avec btrim pour refuser
 *      les chaînes composées uniquement d'espaces.
 *   8. measure du journal contrainte à être positive ou nulle.
 *   9. Version d'empreinte ajoutée, pour permettre la rotation du secret HMAC.
 *
 * PÉRIMÈTRE : données et invariants de ligne uniquement. Ni service, ni route,
 * ni worker. Le service transactionnel est le lot 1B.
 *
 ...
 * l'écriture : vocabulaires fermés ; compteur de relance à 0 ou 1 ; horodatages
 * exigés par l'état, dans un ordre chronologique cohérent ; destinataire figé et
 * compte validateur existant ; delivery_allowed impossible à mettre à vrai ;
 * empreintes au format d'empreinte ; aucun texte libre dans le journal ;
 * longueurs bornées ; aucun dossier orphelin.
 *
 * NON garanti par ce schéma :
 *   · la légalité d'une TRANSITION d'état — PROSPECTION_TRANSITIONS est une
 *     règle applicative ; un UPDATE direct réussirait. La garantie repose sur le
 *     service transactionnel, seul détenteur du droit d'écriture, avec mise à
 *     jour conditionnée par (id, state, revision).
 *   · le RÔLE du compte validateur — la clé étrangère prouve que le compte
 *     existe, pas qu'il est actif ni administrateur. Une contrainte de ligne ne
 *     peut pas consulter proprement une autre table : le service doit le
 *     vérifier.
 ...
 *     l'activation accidentelle du champ ; l'absence d'envoi dépend AUSSI de la
 *     règle d'architecture interdisant tout worker de lecture de subject/body
 *     vers le service d'expédition.
 ...
 *     toujours le recopier depuis le dossier chargé dans la transaction, jamais
 *     depuis une requête cliente.
 */
...
export const PROSPECTION_STATES = [
  ...
  "draft_ready", // accroche validée par un compte administrateur
  ...
  "no_website",
  "opposed", // terminal, irréversible, atteignable à tout moment
  "abandoned", // terminal, atteignable à tout moment
] as const;
...
/* -------------------------------------------------------------------------- */
/* 2. Groupes d'états — utilisés par les contraintes                          */
/* -------------------------------------------------------------------------- */

/**
 * États nécessairement atteints après vérification de cohérence.
 * `opposed` et `abandoned` en sont exclus : ils sont atteignables à tout moment,
 * y compris depuis `opened`.
 */
...
/** États nécessairement atteints après validation humaine de l'accroche. */
...
/** États nécessairement atteints après mise en file proposée. */
...
/** États nécessairement atteints après envoi du premier contact. */
...
/** États où aucune relance n'a encore pu avoir lieu. */
...
/** États vivants : le dossier n'est pas clos. */
const LIVE_STATES = [
  "opened",
  "verified",
  "draft_ready",
...
/* -------------------------------------------------------------------------- */
/* 3. Fabriques de contraintes                                                */
/* -------------------------------------------------------------------------- */

/** `colonne IN ('a', 'b', …)` — valeurs paramétrées, jamais concaténées. */
...
/**
 * Longueur du texte utile, espaces extérieurs retirés.
 * Refuse donc une chaîne composée uniquement d'espaces.
 */
function trimmedLength(column: AnyPgColumn, min: number, max: number): SQL {
  return sql`length(btrim(${column})) BETWEEN ${min} AND ${max}`;
}
...
/** Nombre maximal d'indices de cohérence conservés par dossier. */
 ...
 * seraient plus retrouvées. Le secret ne doit jamais être stocké en base ni
 * figurer dans le journal.
 *
 * Rotation : les colonnes de version d'empreinte permettent de faire coexister
 * plusieurs générations. Une opposition enregistrée sous la version 1 reste
 * opposable ; le service compare l'adresse candidate à chaque version présente.
 */
...
export const prospectionDossiers = pgTable(
  "prospection_dossiers",
  {
    ...
    /* --- accroche : production et validation, séparées -------------------- */

    hook: text("hook"),

    /** Origine de PRODUCTION : signals, human ou ai. Ne change pas à la relecture. */
    ...
    /** Compte administrateur ayant validé l'accroche. */
     ...
     * Leur nombre s'obtient par cardinality() : aucun compteur redondant.
     */
  ...
  (table) => [
    ...
    /* --- longueurs et chaînes non vides ---------------------------------- */
    check(
    ...
    check(
      ...
      sql`cardinality(${table.verifiedSignals}) <= ${MAX_SIGNALS}`,
    ),
    ...
    check(
      ...
      whenState(table.state, PRE_FOLLOW_UP_STATES, sql`${table.followUpCount} = 0`),
    ),
    check(
    ...
    // Une validation est complète ou absente, jamais à moitié.
    check(
    ...
    // Une réponse ne précède pas l'envoi qui l'a provoquée.
    check(
    ...
    // Une clôture post-envoi est postérieure à l'envoi.
    check(
...
/**
 * Messages validés par un compte administrateur, prêts à être copiés et
 * expédiés à la main.
 *
 * Ce n'est PAS l'outbox transactionnel d'expédition. Deux protections
 * distinctes, à ne pas confondre :
 *   · structurelle — delivery_allowed = false empêche l'activation accidentelle
 *     du champ par une simple mise à jour ;
 ...
 *
 * onDelete: "restrict" sur le dossier : supprimer un dossier ne doit pas
 * effacer la preuve qu'un message a été validé et proposé. Traiter les
 * propositions est donc une décision de rétention, pas un effet de bord.
 */
export const prospectionProposals = pgTable(
  "prospection_proposals",
  {
    ...
    /**
     * Empreinte du destinataire au moment de la validation.
     *
     * Fige la proposition sur la personne pour laquelle le contenu a été relu.
     * Si l'adresse du dossier change, l'empreinte ne correspond plus : le
    ...
    /**
     * Compte administrateur ayant validé.
     *
     * La clé étrangère prouve que le compte existe. Elle ne prouve ni qu'il est
     * actif, ni qu'il porte le rôle administrateur : le service doit le
     * vérifier, et ne jamais accepter cette valeur depuis le corps de la requête.
     */
  ...
  (table) => [
    ...
    check(
      ...
      sql`${table.recipientHmacVersion} >= 1`,
    ),
    check(
 ...
 * convention figée plus haut. Le résultat reste une pseudonymisation et non une
 * anonymisation, mais il résiste à l'énumération hors ligne — ce qu'un SHA-256
 * nu ne fait pas.
 *
 * La clé primaire porte sur le couple (empreinte, version) : une rotation du
 * secret n'efface pas les oppositions historiques.
 */
export const prospectionOppositions = pgTable(
  ...
  (table) => [
    uniqueIndex("prospection_oppositions_pkey").on(table.emailHmac, table.hmacVersion),
    ...
    check("prospection_oppositions_version_positive", sql`${table.hmacVersion} >= 1`),
    check(
...
/**
 * Aucune colonne de texte libre : vocabulaires fermés, slug au format
 * strictement contraint, entier de mesure positif.
 *
 * Le format du slug interdit « @ » et l'espace : un appel mal formé ne peut donc
 * pas y inscrire une adresse ou un message. Le service doit de plus n'y recopier
 * que le slug du dossier chargé dans la transaction, jamais une valeur venue
 * d'une requête cliente.
 *
 * Append-only par convention transactionnelle : UPDATE et DELETE restent
 * techniquement possibles, à verrouiller par les permissions du rôle applicatif
 * ou par un trigger si l'immuabilité doit être garantie.
 */
export const prospectionJournal = pgTable(
  ...
  (table) => [
    ...
    check(
      ...
      sql`${table.measure} IS NULL OR ${table.measure} >= 0`,
    ),
  ],
);
 ...
 * structure : un `UPDATE ... SET state = 'enrolled'` direct réussirait, même si
 * les contraintes d'horodatage le rendraient difficile. Elle n'a de valeur que
 * si toutes les mutations passent par le service transactionnel, avec une mise à
 * jour conditionnée par (id, state, revision), et si aucun autre composant ne
 * détient de droit d'écriture sur la table.
 */
...
/* -------------------------------------------------------------------------- */
/* 11. Contraintes critiques — pour le contrôle de dérive                      */
/* -------------------------------------------------------------------------- */

/**
 * À ajouter à expectedNonSn13Constraints dans lib/db/src/schema-check.ts, en
 * important cette constante plutôt qu'en recopiant les chaînes : la liste reste
 * ainsi solidaire du schéma qu'elle protège.
 *
 * ⚠️ Les définitions ci-dessous sont une intention, écrite en SQL lisible.
 * PostgreSQL normalise les expressions de contrainte à sa manière — ajout de
 * parenthèses, réécriture de BETWEEN, qualification des colonnes. Conformément
 * à la règle du projet consistant à ne jamais supposer que Drizzle a
 * correctement remplacé les contraintes actives, il faut relever les
 * définitions réelles après la première migration :
 *
 ...
 *
 * puis recopier les valeurs obtenues dans ce tableau. Toute divergence
 * ultérieure sera alors détectée par le contrôle de dérive existant.
 */
export const PROSPECTION_CRITICAL_CONSTRAINTS = [
  {
    tableName: "prospection_dossiers",
    name: "prospection_dossiers_state_known",
    definition: "CHECK (state IN (…états déclarés…))",
  },
  {
    tableName: "prospection_dossiers",
  ...
  },
  {
    ...
    definition: "CHECK (NOT (state IN (…états pré-relance…)) OR (follow_up_count = 0))",
  },
  {
    tableName: "prospection_dossiers",
    name: "prospection_dossiers_hook_required",
    definition:
      "CHECK (NOT (state IN (…états post-vérification…)) OR (hook IS NOT NULL AND hook_origin IS NOT NULL))",
  },
  {
    tableName: "prospection_dossiers",
    name: "prospection_dossiers_hook_validation_required",
    definition:
      "CHECK (NOT (state IN (…états post-brouillon…)) OR (hook_validated_at IS NOT NULL AND hook_validated_by_account_id IS NOT NULL))",
  },
  {
    tableName: "prospection_dossiers",
    name: "prospection_dossiers_contact_required",
    definition:
      "CHECK (NOT (state IN (…états post-proposition…)) OR (contact_email IS NOT NULL))",
  },
  {
    tableName: "prospection_dossiers",
    name: "prospection_dossiers_sent_at_required",
    definition: "CHECK (NOT (state IN (…états post-envoi…)) OR (sent_at IS NOT NULL))",
  },
  {
    tableName: "prospection_dossiers",
    name: "prospection_dossiers_slug_format",
    definition: "CHECK (slug ~ '^[a-z0-9]([a-z0-9-]{0,118}[a-z0-9])?$')",
  },
  {
  ...
  },
  {
    ...
    definition: "CHECK (recipient_hmac ~ '^[0-9a-f]{64}$')",
  },
  {
    tableName: "prospection_proposals",
  ...
  },
  {
    ...
    definition: "CHECK (email_hmac ~ '^[0-9a-f]{64}$')",
  },
  {
    tableName: "prospection_oppositions",
    name: "prospection_oppositions_origin_known",
    definition:
      "CHECK (origin IN ('email_reply', 'admin_entry', 'telegram_callback', 'unsubscribe_request'))",
  },
  {
    tableName: "prospection_journal",
    name: "prospection_journal_event_known",
    definition: "CHECK (event IN (…événements déclarés…))",
  },
  {
    tableName: "prospection_journal",
    name: "prospection_journal_slug_format",
    definition: "CHECK (slug IS NULL OR slug ~ '^[a-z0-9]([a-z0-9-]{0,118}[a-z0-9])?$')",
  },
  {
    tableName: "prospection_journal",
